  ...
  <div v-if="isVisible" class="modal">
    <div class="modal-content">
      ...
      <form @submit.prevent="submit">
        ...
        <br>
        <div class="btn-group">
          <v-btn type="submit" class="bg-success">Save</v-btn>
...
<script setup>
    ...
    const props = defineProps(['isVisible', 'user', 'users']);
    ...
    errors.value = {};
    if (!user.value.Name) {
        errors.value.Name = 'Tên không được đê trống.';
    }
    if (!user.value.Email) {
        errors.value.Email = 'Email không được để trống.';
    } else if (!validateEmail(user.value.Email)) {
        errors.value.Email = 'Email không hợp lệ.';
    } else if (props.users.some(item => item.Email === user.value.Email && item.id !== user.value.id)) {
